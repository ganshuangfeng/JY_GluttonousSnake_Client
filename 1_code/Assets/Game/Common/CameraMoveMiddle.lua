...
CameraMoveMiddle = basefunc.class()
local M = CameraMoveMiddle
local is_debug = true
local instance

local base_size = 11.7
local base_show_size = {min_x=-3, max_x=3, min_y=-11, max_y=9,} -- 显示区域大小
...
function M:FrameUpdate(time_elapsed)
	...
	-- 视口在地图内并且物体在自由移动区域
	if (cam_pos.x + self.show_size.min_x) > (self.map_size.min_x - 0.0001)
		and (cam_pos.x + self.show_size.max_x) < (self.map_size.max_x - 0.0001)
		and (cam_pos.y + self.show_size.min_y) > (self.map_size.min_y - 0.0001)
		and (cam_pos.y + self.show_size.max_y) < (self.map_size.max_y - 0.0001)
	...
	-- 视口在地图外 -- 矫正
	if (x + self.show_size.min_x) <= (self.map_size.min_x + 0.0001) then
		x = self.map_size.min_x - self.show_size.min_x
		dd = dd .. "+11"
	elseif (x + self.show_size.max_x) >= (self.map_size.max_x + 0.0001) then
		x = self.map_size.max_x - self.show_size.max_x
		dd = dd .. "+12"
	end
	-- 视口在地图外 -- 矫正
	if (y + self.show_size.min_y) <= (self.map_size.min_y + 0.0001) then
		y = self.map_size.min_y - self.show_size.min_y
		dd = dd .. "+13"
	elseif (y + self.show_size.max_y) >= (self.map_size.max_y + 0.0001) then
	...
	if CameraMoveBase.IsDebug then
		...
		if Vec2DLength( {x=_ppp.x-CSPanel.camera3d.transform.position.x, y=_ppp.y-CSPanel.camera3d.transform.position.y} ) > 0 then
