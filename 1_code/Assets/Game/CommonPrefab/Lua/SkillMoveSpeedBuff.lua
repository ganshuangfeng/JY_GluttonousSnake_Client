...
SkillMoveSpeedBuff = basefunc.class()
local M = SkillMoveSpeedBuff
M.name = "SkillMoveSpeedBuff"
...
function M.CreateBefore(data)
    ...
    if skills and next(skills) then
        for k,skill in pairs(skills) do
            dump(skill)
            if skill.data.type == "move_speed_buff" then
...
function M:OnCreate()
    ...
    elseif self.skill_cfg.effect_hero == "random_grid" then
        ...
        self:SetComponsePanelTX(self.random_grid,"hero_move_speed_up_fx","on_panel")
...
function M:OnTrigger()
    ...
    fx_pre.transform:GetComponent("Image").sprite = GetTexture("2d_word_pz_7")
...
function M:SetAttribute()
    local vehicle = GameInfoCenter.GetHeroHead().vehicle
...
function M:ResetAttribute()
    if self.old_vehicle_data and next(self.old_vehicle_data) then
        local vehicle = GameInfoCenter.GetHeroHead().vehicle
        vehicle:SetMaxSpeed(self.old_vehicle_data.m_dMaxSpeed)
...
function M:RefreshHeroOnLocation()
    if self.random_grid then
        ...
        self.tx_3d_map = {}
        if effecter and IsEquals(effecter.transform) then
            self.tx_3d_map[effecter.data.heroId] = NewObject("GX_guang",effecter.transform)
        end
        ...
        for k,v in pairs(self.effecters) do
            if v.data then
                ...
                if hero_wait then
                    self:SetComponsePanelTX(hero_wait,"hero_move_speed_up_fx_1","on_hero")
                end
            end
        end
    end
...
function M:CloseComponsePanelTX(pos,pre_name)
    -- if not self.tx_map then return end
    -- local componse_panel = CSPanel.componse_pre
    -- componse_panel:SetTX(pos,pre_name,false)
    -- for k,v in ipairs(self.tx_map) do
    --     if v.pos == pos and v.pre_name == pre_name then 
    --         table.remove(self.tx_map,k)
    --         break
    --     end
    -- end
