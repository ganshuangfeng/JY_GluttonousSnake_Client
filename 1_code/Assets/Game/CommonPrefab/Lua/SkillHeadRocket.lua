...
function M:MakeLister()
    self.lister = {}
	self.lister["ui_game_get_jin_bi_msg"] = basefunc.handler(self,self.OnMgrSkillHeadCountChange)
end
...
function M:RefreshSkillPreTblUI()
    if self.triggerCount >= self.requireTriggerCount then
        ...
        self.skillPreTbl.rocket_skill_img.fillAmount = self.triggerCount / self.requireTriggerCount
        if self.triggerCount > 0 then
            ...
            local progress = self.triggerCount / self.requireTriggerCount
        ...
        self.skillPreTbl.rocket_skill_txt.text = self.triggerCount .. "/" .. self.requireTriggerCount
