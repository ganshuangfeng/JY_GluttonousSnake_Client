-- 创建时间:2021-05-26
local cur_path = "Game.game_Hall.Lua."
ExtRequire(cur_path .. "HallModel")
ExtRequire(cur_path .. "HallGamePanel")
...
function L.Init(parm)
    this = L
    dump(parm, "<color=red>HallLogic Init parm</color>")
    --初始化model
    local model = HallModel.Init()
    MakeLister()
    AddMsgListener(lister)
...
function L.Exit()
    if this then
        print("<color=green>Exit  HallLogic</color>")
        this = nil
        if cur_panel then
...
function L.change_panel(panelName)
    ...
    if not cur_panel then
        ...
        elseif panelName == L.panelNameMap.game then
            cur_panel = {name = panelName, instance = HallGamePanel.Create()}
        end
    end
end
