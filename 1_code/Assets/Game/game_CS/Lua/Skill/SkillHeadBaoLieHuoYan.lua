----张开大嘴对前方持续喷出爆裂火焰持续10秒，被爆裂火焰击中的敌人受到每秒100的灼烧伤害，持续5秒。
----道具CD：20秒

local basefunc = require "Game/Common/basefunc"

SkillHeadBaoLieHuoYan = basefunc.class(Skill)
local M = SkillHeadBaoLieHuoYan
function M:Ctor(data)
    M.super.Ctor(self, data)
    self.data = data
    self.heroHeadSpModifier = nil
	self.addTagName = "BaoLieHuoYan"
...
function M:AddTX()
    ...
    if not self.tx_wei then
        ...
        local t2 = NewObject("CT_ST_hongse_gx",heroHead.transform)
    ...
    for key , h in pairs(all_hero) do
        ...
        if not self.tx_shenti[h.data.location] then
            local tx_name = {"CT_ST_hongse_gx","CT_ST_hongse_gx"}
...
function M:PenHuo(dt)
    ...
    if self.left_time <= 0 then
        ...
        hit_data.bulletPrefabName = {"snake_penhuo"}
