...
SkillLightHitSuperAttack = basefunc.class(Skill)
local M = SkillLightHitSuperAttack
...
function M:AttackTarget()
	...
	if not isCan then
		base_pos = self.object.transform.position + Vector3.New(math.random(-2,2),math.random(-2,2))
	else
        base_pos = ObjectCenter.GetObj(self.attackTargetId).transform.position
    end
    local hit_data = {}
    hit_data.speed = 25
    ...
    hit_data.hitOrgin = {pos = base_pos + Vector3.New(3,30,0)}
