...
SkillMonsterShaMoMengChongAttack = basefunc.class(Skill)
local M = SkillMonsterShaMoMengChongAttack
...
function M:OnTrigger()
    ...
    if self.object.anim_pay then
		self.object.anim_pay:Play("Monster_shamomengchong_attack",0,0)
	end
    Timer.New(
        function()
            if self.object.isLive then
                self:AttackTarget()
                if self.object.anim_pay then
                    self.object.anim_pay:Play("Monster_shamomengchong_idel",0,0)
                end 
            end
        end,0.8,1
    ):Start()
