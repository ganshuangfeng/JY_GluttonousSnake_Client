...
MonsterBossSuperAttackCtrl = basefunc.class(BaseCtrl)

local M = MonsterBossSuperAttackCtrl
...
function M:Begin()
	if self.ctrlState==StateType.ready then
        ...
        --print("xxx-----------------boss super attack begin")
        --- 播放动画
        if IsEquals(self.object.anim_pay) then
            self.object.anim_pay:Play(self.animName, 0, 0)
        end
...
--- 结束时调用
function M:Finish()
   -- print("xxx-----------------boss super attack finish")
...
--强制结束
function M:Stop()
    --print("xxx-----------------boss super attack stop")
    --发送强制结束信号
