...
namespace SWS
{
    ...
    public class PathManager : MonoBehaviour
    {
        ...
        private void UpdatePosConversion(Transform[] waypoints)
        {
            for (int i = 0; i < waypoints.Length; i++)
            {
                var tf = waypoints[i];
                var posConversion = tf.GetComponent<PosConversion>();
                if (posConversion == null)
                {
                    posConversion = tf.gameObject.AddComponent<PosConversion>();
                }
                posConversion.ConversionPos();
            }
        }
    }
}
