...
public class LuaUtility
{
    ...
    public static bool IsShowLog { get { return isShowLog;} set {isShowLog = value;} }
    ...
    private static string ToLua(object obj, int level, Type obj_member_type, ToLuaSetting setting)
    {
        ...
        else
        {
            ...
            var contractAttr = System.Attribute.GetCustomAttributes(type); //反射获得用户自定义属性
            //如果类有 DataContract 特性的话，就只导出其带有 DataMember 特性的字段，否则导出所有 public 字段
            // bool isNeedAttr = contractAttr != null;
            bool isNeedAttr = false;
            foreach (System.Attribute attr in contractAttr)
            {
                    ...
                    if (isNeedExport)
                    {
                        ...
                        else if (setting == null || !setting.SkipProperty)
                        {
                            ...
                            if (pro != null && pro.CanRead && pro.CanWrite)
                            {
                                ...
                                try {
                                    obj_value = pro.GetValue(obj,null);
                                }
                                catch{}
                            }
                        }
    ...
    public class LexState : ICloneable
    {
        ...
        private void OnlySave(ref Token _token, string str)
        {
            _token.str = _token.str.Substring(0, _token.str.Length-1);
            _token.str += str;
        }
        private void ReadSave(ref Token _token, string str)
        {
        ...
        else
        {
            ...
            if (mems != null && mems.Length == 1 && val != null)
            {
                ...
                else
                {
                    ...
                    if (pro != null)
                    {
                        pro.SetValue(obj, ConvertToRealType(val, fieldType),null);
                    }
                }
            }
            else
